...
def plot_embedding(data, label, title):
    ...
    for i in range(data.shape[0]):
        ...
        # 在图中为每个数据点画出标签
        if label[i] == 0:
            s1 = plt.text(data[i, 0], data[i, 1], 'o', color=plt.cm.Set1((label[i] / 10)+0.3),
                     fontdict={'weight': 'bold', 'size': 7},size=7,label='Square function')
        if label[i] == 1:
            s2 = plt.text(data[i, 0], data[i, 1], '*', color=plt.cm.Set1((label[i] / 10)+0.1),
                     fontdict={'weight': 'bold', 'size': 7},size=9,label='Square function')
